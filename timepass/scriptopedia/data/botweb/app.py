...
BOT_SCRIPT_PATH = os.path.abspath(os.path.join(BASE_DIR, '..', 'bot.sh'))
    ...
    def do_GET(self):
        ...
        <head>
            ...
            <style>
                ...
                input {{ background: #000; color: #0f0; border: 1px solid #0f0; width: 70%; padding: 10px; transition: all 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94);}}
        ...
        <img class="imgS" src="img/LOADING.gif" onClick="return(view());" title="About Us"/>
            <h2>Terminal BOT<span class="loader__dot">.</span><span class="loader__dot">.</span><span class="loader__dot">.</span></h2>
            ...
            <script>
                ...
                setInterval(() => {{
                    fetch('/status').then(r => r.text()).then(data => {{
                        ...
                        term.innerHTML = ansiToHtml(data);
