        ...
        try:
            ...
            for entry in conf_list:
                ...
                for nw in entry[cat_key]:
                    ...
                    # Get icon path or use a transparent spacer if missing
                    icon_src = nw.get('imgIcon', '')
                    icon_html = f'<img src="{icon_src}" class="target-icon" style="width:20px; height:20px; margin-right:8px; vertical-align:middle;">' if icon_src else ""
                    ...
                    <div class="target-box" data-status="{status}" data-search="{nw['name'].lower()} {nw['ip']}"
                        ...
                        <div class="target-header" style="display: flex; align-items: center; justify-content: space-between;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                {icon_html}
                                <span class="target-label" style="white-space: nowrap;">{nw['name']}</span>
                            </div>
                            <div class="status-indicator"></div>
                        </div>
                        <p class="target-subtext" style="margin-top: 4px;">{nw['ip']}:{nw['port']}</p>
                    </div>'''
...
class Handler(http.server.SimpleHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        # This ensures the server can serve the /icons folder if it's inside /templates or BASE_DIR
    ...
    def do_GET(self):
        ...
        # If accessing the root, serve index.html from templates
